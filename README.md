...
- The research report now includes a file-explorer style Research Artifact Explorer that groups the typed bundle into search frame, proposals, comparisons, final decision, and runtime evidence. Selecting an item opens an in-page details pane, including full markdown rendering for technical dossiers and final memo artifacts.
...
- The observer memory ledger now shows provider-routing history as a provider x action matrix and lets operators filter reusable priors by outcome-backed, mixed-evidence, or search-only provenance instead of collapsing the shared memory view into provider-level totals.
