...
- Research and staged final-decision authoring now retry once with explicit schema-repair feedback when a provider returns a `FinalDecisionPackage` that either leaves dangling `final_decision_doc` references or invents proposal ids not present in the run bundle, keeping the validator strict while recovering from common final-authoring id-alignment mistakes.
