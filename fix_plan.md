...
- The adaptive frontier loop now reflects the paper-inspired runtime shape better than the earlier scripted phase chain, but it still does not implement explicit archive parent sampling, island migration, or outcome-backed policy updates. Those remain the main gaps between the current runtime and the archive conversation’s later-stage search behavior.
