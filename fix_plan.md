...
- [x] Add Claude Code, Gemini CLI, and OpenCode provider adapters behind the same interface, while keeping Codex as the default and best-supported provider.
...
- The default `standard` and `max` cost profiles now both allow up to 8 concurrent provider tasks, while `lean` stays at 2 so low-cost runs still bound provider fan-out.
...
- The provider layer now supports `codex`, `claude`, `gemini`, and `opencode` behind the same typed contract. Codex remains the default and the most strongly supported path, while Claude Code, Gemini, and OpenCode use their headless JSON surfaces plus the same audited prompt, artifact, and schema-validation pipeline.
- Gemini headless invocations now stream the materialized prompt over stdin while using an empty `--prompt` flag to select non-interactive mode, which keeps large research prompts from failing at the OS argv limit.
- Run-config provider blocks now support `reasoning_effort` for Codex-backed entries, and the Codex adapter forwards that per-provider override as `model_reasoning_effort` instead of forcing operators to rely on global Codex config.
...
- Run-config files now also support `runtime_mode`, so research-oriented prompt files can select the coverage-led artifact pipeline without requiring a separate CLI flag.
- Run-config provider blocks now support Codex-only `service_tier`, so operators can request `service_tier = "fast"` alongside explicit model and reasoning-effort overrides.
- Run-config provider blocks now support Codex-only `web_search`, so source-heavy research runs can pass `--search` to `codex exec` while keeping provider prompts explicit about when web-grounded evidence is allowed.
...
- The repository now includes an optional `flake.nix` dev shell for Linux/WSL that supplies `python3.12`, `uv`, `node`, and `npm` while preserving the existing `uv run ...` and `./scripts/verify.sh` operator workflow inside the shell.
...
- Research and staged final-decision authoring now retry once with explicit schema-repair feedback when a provider returns a `FinalDecisionPackage` that either leaves dangling `final_decision_doc` references or invents proposal ids not present in the run bundle, and research-mode now normalizes provider-framed ledgers back to a real pre-seeding state so scheduling cannot stop before any `ProposalBrief` exists.
...
- The live observer run page now defaults research-mode runs to a `research_bundle`-driven concept board that separates the search frame from seeded concepts, shortlist survivors, and eliminated families, while preserving the raw node DAG as an `Execution Graph` toggle for debugging novelty/admission behavior.
- Latest-run inspection on `run-20260517T173138Z` showed that research proposals and deep dives were too shallow for math-heavy source-domain-transfer work: proposal files were only short summaries and deep dives did not preserve the underlying formulas, implementation hooks, ablation design, or prior-art collision analysis. Research `DeepDiveDoc` now includes a standalone `technical_dossier_markdown` field, provider schemas require it for new deepening actions, and rendered runs write the dossier to `research/markdown/deep-dives/technical/<doc_id>.md`.
- The final report now includes a Research Artifact Explorer: a file-tree style view over search frame, proposal folders, deep dives, technical dossiers, reviews, comparisons, final decision artifacts, and scheduler evidence. Clicking an artifact opens a details pane instead of navigating away.
