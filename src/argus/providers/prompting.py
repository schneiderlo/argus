...
def _deepen_family_instructions(
    ...
    instructions = [
        ...
        "Technical dossier rule: technical_dossier_markdown is the durable file for the underlying work behind the proposal. It must not be a slogan or summary. Write it as a standalone technical note with enough math, mechanism, code-hook detail, and ablation design that a serious implementer can evaluate the idea without rereading the rest of the run.",
        "Technical dossier sections: include at least Mechanism Translation, Mathematical Rule or Formula, Nanochat Integration Points, Minimal Implementation Sketch, Ablation Design, Expected Signals, Failure Modes, Prior-Art Collision, and Verification Needed. Use equations, pseudocode, file/function targets, and explicit uncertainty where relevant.",
        "Depth bar: if the proposal comes from a mathematical or scientific source domain, explain the actual transferable rule and its assumptions, not only the analogy. For source-dependent claims, cite only sources from the input or mark them as verification_needed.",
...
def _write_final_decision_instructions(
    ...
    instructions = [
        ...
        "Technical-depth rule: when deep_dive_docs include technical_dossier_markdown, use those dossiers as the source of truth for mathematical mechanism, code hooks, ablation design, prior-art collision, and verification-needed caveats. Do not collapse a proposal into a one-phrase proposition when a dossier exists.",
